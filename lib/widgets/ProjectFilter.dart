...
class _FilterModel extends State<FilterModel> {
  // 区域相关
  List region_name_list = []; //区级数据列表
  List area_name_list = []; //街道相关列表
...
// 价格相关
  ...
  void _getInitData(context) async {
    ...
    setState(() {
      ...
      _region_name_list.retainWhere((element) {
        List _item = element['children']; //市级
  ...
  @override
  void didUpdateWidget(FilterModel oldWidget) {
  ...
  Widget build(BuildContext context) {
    Widget Render = Container(height: 0, width: 0);
    if (firstRequest) {
                      ...
                      return GestureDetector(
                        ...
                        onTap: () {
                          widget.changeSendParams(
                            params: {'type': 'cur_region', 'val': i['name']},
                          );
                          setState(() {
                            getAreaList(i['name']);
                          });
                  ...
                  child: Column(
                    ...
                    children: area_name_list.map((i) {
                      return GestureDetector(
                        ...
                        onTap: () {
                          widget.changeSendParams(
                            params: {'type': 'cur_area', 'val': i['name']},
                          );
                          widget.retrieveDataFilter();
                        },
                      );
                          ...
                          child: Container(
                            ...
                            child: Center(
                              child: Text(
                                ...
                                style: TextStyle(
                                    color:
                                        cur_area == "全部" ? Colors.blue : null),
                              ),
                            ),
                          ),
                          onTap: () {
                            widget.changeSendParams(
                              params: {'type': 'cur_area', 'val': "全部"},
                            );
                            // 数据请求（全部）
                            widget.retrieveDataFilter();
                          },
              ...
              return Column(children: [
                GestureDetector(
                  child: Center(
                    child: Text(
                      i,
                      style: TextStyle(color: room == i ? Colors.blue : null),
                    ),
                  ),
                  onTap: () {
                    widget.changeSendParams(params: {'type': 'room', 'val': i});
                    widget.retrieveDataFilter();
                  },
                ),
                Divider(
    ...
    return widget.modelState != null && widget.modelState > 0
        ? Positioned(
            top: 75.0,
            left: 0,
            right: 0,
